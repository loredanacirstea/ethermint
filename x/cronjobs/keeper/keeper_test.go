...
func (suite *KeeperTestSuite) TestBaseFee() {
	...
	sender, err := sdk.AccAddressFromHex(suite.address.Hex())
	...
	registerMsg := &cronjobstypes.MsgRegisterCronjob{
		Sender:  sender.String(),
		Cronjob: cronjob,
	}
	...
	suite.app.CronjobsKeeper.RegisterCronjob(ctx, registerMsg)

	job, found := suite.app.CronjobsKeeper.GetCronjob(suite.ctx, suite.app.CronjobsKeeper.GetId(sender.String(), "1"))
	suite.Require().True(found)
	suite.Require().Equal(cronjob.Identifier, job.Identifier)

	// suite.CommitAfter(time.Hour * 2)
